...
class CadastroMedicoState extends State<CadastroMedico> {
  ...
  Future<void> _navegarParaAlocacao() async {
    ...
    final disponibilidadesUnicasParaVerificar = disponibilidades
      ...
      if (seriesCarregadas.isNotEmpty) {
        ...
        for (final disp in this.disponibilidades) {
          if (disp.tipo == 'Única' && disp.medicoId == medicoId) {
            ...
            debugPrint(
                '🔒 Preservando disponibilidade única não salva: ${disp.data.day}/${disp.data.month}/${disp.data.year}');
          }
        }

        // SEGUNDO: Adicionar todas as disponibilidades existentes que não são do ano atual
        // e que não são únicas (já foram adicionadas acima)
        for (final disp in this.disponibilidades) {
          if (disp.data.year != anoParaCarregar && disp.tipo != 'Única') {
            final chave =
                '${disp.medicoId}_${disp.data.year}-${disp.data.month}-${disp.data.day}_${disp.tipo}';
            // Só adicionar se não é única (únicas já foram adicionadas acima)
            if (!disponibilidadesFinais.containsKey(chave)) {
              disponibilidadesFinais[chave] = disp;
            }
          }
        }

        // TERCEIRO: Adicionar as disponibilidades geradas/ordenadas (séries do ano atual + únicas do Firestore)
        // IMPORTANTE: Não sobrescrever disponibilidades únicas que já existem (ainda não salvas)
        // Isso garante que disponibilidades recém-adicionadas não sejam perdidas
        for (final disp in listaOrdenada) {
          ...
          if (disp.tipo == 'Única' &&
              disponibilidadesFinais.containsKey(chave)) {
            debugPrint(
                '⚠️ Disponibilidade única já existe localmente (não salva), preservando: $chave');
        ...
        try {
          ...
          for (final disp in this.disponibilidades) {
            if (disp.tipo == 'Única' && disp.medicoId == medicoId) {
              ...
              disponibilidadesUnicas[chave] = disp;
              debugPrint(
                  '🔒 Preservando disponibilidade única não salva ao carregar: ${disp.data.day}/${disp.data.month}/${disp.data.year}');
            }
          }

          // SEGUNDO: Adicionar/sobrescrever com as do Firestore (já salvas)
          // IMPORTANTE: Sobrescrever apenas se a disponibilidade do Firestore existe (já foi salva)
          // Isso permite que disponibilidades recém-salvas substituam as locais
          for (final dispUnica in dispsUnicas) {
            final chave =
                '${dispUnica.medicoId}_${dispUnica.data.year}-${dispUnica.data.month}-${dispUnica.data.day}_${dispUnica.tipo}';
            // Se já existe, substituir pela do Firestore (já foi salva, então é mais atualizada)
  ...
  Future<void> _salvarMedico() async {
    ...
    // CORREÇÃO CRÍTICA: Capturar disponibilidades únicas ANTES de qualquer operação
    // que possa alterar a lista disponibilidades (como salvarMedicoCompleto)
    // Isso garante que disponibilidades únicas sejam salvas mesmo se a lista for modificada
    // IMPORTANTE: Criar cópias profundas para garantir que não sejam afetadas por alterações na lista original
    debugPrint(
        '🔍 [_salvarMedico] Verificando disponibilidades únicas na lista atual (total: ${disponibilidades.length})');
    // CORREÇÃO: Fazer cópia PROFUNDA da lista para garantir que não seja afetada por modificações
    final todasDisponibilidades = disponibilidades
        .map((d) => Disponibilidade(
              id: d.id,
              medicoId: d.medicoId,
              data: d.data,
    ...
    // Debug: mostrar TODAS as disponibilidades antes de filtrar
    debugPrint(
        '   🔍 Total de disponibilidades na lista: ${todasDisponibilidades.length}');
    for (final disp in todasDisponibilidades) {
      debugPrint(
          '     - Tipo=${disp.tipo}, ID=${disp.id}, data=${disp.data.day}/${disp.data.month}/${disp.data.year}');
    }

    final disponibilidadesUnicasParaSalvar = todasDisponibilidades
        .where((d) => d.tipo == 'Única' && d.medicoId == _medicoId)
        .map((d) {
      // CORREÇÃO: Se ID estiver vazio ou for temporário, gerar um novo permanente antes de salvar
      String idFinal = d.id;
      if (idFinal.isEmpty ||
          idFinal.startsWith('temp_') ||
          idFinal.endsWith('-0')) {
        final timestamp = DateTime.now().microsecondsSinceEpoch;
        final dataStr =
            '${d.data.year}${d.data.month.toString().padLeft(2, '0')}${d.data.day.toString().padLeft(2, '0')}';
        idFinal = '${timestamp}_${_medicoId}_$dataStr';
        debugPrint(
            '   ⚠️ ID vazio ou temporário (${d.id}) detectado, gerando novo permanente: $idFinal');
      }
    ...
    debugPrint(
        '💾 [_salvarMedico] Capturadas ${disponibilidadesUnicasParaSalvar.length} disponibilidades únicas ANTES de qualquer operação');
    for (final disp in disponibilidadesUnicasParaSalvar) {
      debugPrint(
          '   📋 Única capturada: ID=${disp.id}, data=${disp.data.day}/${disp.data.month}/${disp.data.year}, horários=${disp.horarios.isEmpty ? "VAZIO" : disp.horarios}');
    }

    // CORREÇÃO CRÍTICA: Se não há disponibilidades únicas, avisar mas não bloquear
    if (disponibilidadesUnicasParaSalvar.isEmpty) {
      debugPrint(
          '⚠️ [_salvarMedico] NENHUMA disponibilidade única para salvar! Total na lista: ${disponibilidades.length}');
      // Debug: verificar se há disponibilidades únicas na lista original
      final unicasNaLista = todasDisponibilidades
          .where((d) => d.tipo == 'Única' && d.medicoId == _medicoId)
          .length;
      debugPrint(
          '   🔍 Disponibilidades únicas encontradas na lista original: $unicasNaLista');
      if (unicasNaLista > 0) {
        debugPrint(
            '   ⚠️ PROBLEMA: Há $unicasNaLista disponibilidades únicas na lista mas nenhuma foi capturada!');
        // Tentar capturar novamente sem filtro de ID temporário
        for (final disp in todasDisponibilidades
            .where((d) => d.tipo == 'Única' && d.medicoId == _medicoId)) {
          debugPrint(
              '     - ID=${disp.id}, data=${disp.data.day}/${disp.data.month}/${disp.data.year}');
        }
      }
    }

    // CORREÇÃO CRÍTICA: Remover disponibilidades únicas da lista antes de passar para salvarMedicoCompleto
    // porque elas serão salvas separadamente após as séries
    // Isso evita que salvarMedicoCompleto processe disponibilidades únicas com IDs temporários
    final disponibilidadesSemUnicas = disponibilidades
        .where((d) => !(d.tipo == 'Única' && d.medicoId == _medicoId))
        .toList();

    debugPrint(
        '🔍 [_salvarMedico] Removendo ${disponibilidades.length - disponibilidadesSemUnicas.length} disponibilidades únicas da lista antes de salvarMedicoCompleto');
    debugPrint(
        '🔍 [_salvarMedico] Restam ${disponibilidadesSemUnicas.length} disponibilidades (séries/outras) para processar');
    ...
    final medico = Medico(
      ...
      disponibilidades:
          disponibilidadesSemUnicas, // Adiciona as disponibilidades SEM as únicas (para compatibilidade)
    ...
    try {
      ...
      // NOTA: disponibilidades únicas NÃO são processadas aqui - serão salvas depois
      await salvarMedicoCompleto(
        medico,
        unidade: widget.unidade,
        disponibilidadesOriginais: _disponibilidadesOriginal
            .where((d) => !(d.tipo == 'Única' && d.medicoId == _medicoId))
            .toList(), // Também remover únicas das originais
      );
      ...
      // CORREÇÃO: Usar a lista capturada no início, não a lista atual (pode ter sido alterada)
      debugPrint(
          '💾 [_salvarMedico] Total de disponibilidades na lista atual: ${disponibilidades.length}');
      debugPrint(
          '💾 [_salvarMedico] Usando ${disponibilidadesUnicasParaSalvar.length} disponibilidades únicas capturadas no início');

      final disponibilidadesUnicas = disponibilidadesUnicasParaSalvar;

      debugPrint(
          '💾 [_salvarMedico] Salvando ${disponibilidadesUnicas.length} disponibilidades únicas para médico $_medicoId');

      // Debug: mostrar detalhes de cada disponibilidade única
      for (final disp in disponibilidadesUnicas) {
        debugPrint(
            '   📋 Disponibilidade única encontrada: ID=${disp.id}, data=${disp.data.day}/${disp.data.month}/${disp.data.year}, horários=${disp.horarios}');
      }

      for (final disp in disponibilidadesUnicas) {
        try {
          // CORREÇÃO CRÍTICA: Garantir que a disponibilidade única tem um ID válido permanente
          // Verificar se ID é temporário e substituir por permanente
          String idParaSalvar = disp.id;
          if (idParaSalvar.isEmpty ||
              idParaSalvar.startsWith('temp_') ||
              idParaSalvar.endsWith('-0')) {
            final timestamp = DateTime.now().microsecondsSinceEpoch;
            final dataStr =
                '${disp.data.year}${disp.data.month.toString().padLeft(2, '0')}${disp.data.day.toString().padLeft(2, '0')}';
            idParaSalvar = '${timestamp}_${_medicoId}_$dataStr';
            debugPrint(
                '⚠️ Disponibilidade única com ID temporário (${disp.id}), gerando novo permanente: $idParaSalvar');
          }
          ...
          debugPrint(
              '💾 Salvando disponibilidade única: ID=$idParaSalvar, data=${disp.data.day}/${disp.data.month}/${disp.data.year}, ano=$ano, horários=${disp.horarios.isEmpty ? "VAZIO" : disp.horarios}');
          debugPrint('   📋 DataMap completo: $dataMap');

          await disponibilidadesRef.doc(idParaSalvar).set(dataMap);
          unicasSalvas++;
          debugPrint(
              '✅ Disponibilidade única salva com sucesso: $idParaSalvar');
        } catch (e, stackTrace) {
          // CORREÇÃO: Reportar erro ao salvar disponibilidade única
          unicasErros++;
      ...
      debugPrint(
          '💾 [_salvarMedico] Resultado: $unicasSalvas salvas, $unicasErros erros de ${disponibilidadesUnicas.length} disponibilidades únicas');

      // CORREÇÃO: Se deveria ter salvo disponibilidades únicas mas não salvou nenhuma,
      // mostrar aviso (pode indicar problema na lista)
      if (unicasSalvas == 0 && disponibilidadesUnicas.isNotEmpty) {
        debugPrint(
            '⚠️ AVISO: Deveria ter salvo ${disponibilidadesUnicas.length} disponibilidades únicas mas nenhuma foi salva!');
      }
      ...
      // Invalidar cache dos dias das disponibilidades e dos anos relevantes
      final anosInvalidar = <int>{};
      for (final disp in disponibilidades) {
        final d = DateTime(disp.data.year, disp.data.month, disp.data.day);
        AlocacaoMedicosLogic.invalidateCacheForDay(d);
        // Adicionar ano da disponibilidade à lista de anos para invalidar
        anosInvalidar.add(disp.data.year);
      }

      // CORREÇÃO CRÍTICA: Invalidar cache para TODOS os anos das disponibilidades
      // Isso garante que disponibilidades de qualquer ano apareçam no mapa
      ...
      // CORREÇÃO: Invalidar também cache de médicos ativos para garantir recarregamento
      final anoAtual = DateTime.now().year;
      if (!anosInvalidar.contains(anoAtual)) {
        // Se não estava na lista, invalidar ano atual também
  ...
  Future<bool> _salvarMedicoSemSair() async {
    debugPrint(
        '🔍 [_salvarMedicoSemSair] INÍCIO - Total disponibilidades: ${disponibilidades.length}');

    if (!_formKey.currentState!.validate()) {
      debugPrint(
          '❌ [_salvarMedicoSemSair] Formulário inválido - retornando false');
      return false; // Não salva se o formulário for inválido
    }

    // Verifica se o nome foi preenchido
    if (nomeController.text.trim().isEmpty) {
      debugPrint('❌ [_salvarMedicoSemSair] Nome vazio - retornando false');
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Introduza o nome do médico')),
      );
    ...
    debugPrint(
        '✅ [_salvarMedicoSemSair] Validações passaram, capturando disponibilidades únicas...');
    ...
    // IMPORTANTE: Criar cópias profundas para garantir que não sejam afetadas por alterações na lista original
    debugPrint(
        '🔍 [_salvarMedicoSemSair] Verificando disponibilidades únicas na lista atual (total: ${disponibilidades.length})');
    // CORREÇÃO: Fazer cópia PROFUNDA da lista para garantir que não seja afetada por modificações
    final todasDisponibilidades = disponibilidades
        .map((d) => Disponibilidade(
              id: d.id,
    ...
    debugPrint(
        '💾 [_salvarMedicoSemSair] Capturadas ${disponibilidadesUnicasParaSalvar.length} disponibilidades únicas ANTES de qualquer operação');
    for (final disp in disponibilidadesUnicasParaSalvar) {
      debugPrint(
          '   📋 Única capturada: ID=${disp.id}, data=${disp.data.day}/${disp.data.month}/${disp.data.year}, horários=${disp.horarios.isEmpty ? "VAZIO" : disp.horarios}');
    }

    // CORREÇÃO CRÍTICA: Se não há disponibilidades únicas capturadas, verificar se há na lista original
    // Pode ser que o filtro tenha falhado ou as disponibilidades não foram capturadas corretamente
    if (disponibilidadesUnicasParaSalvar.isEmpty) {
      debugPrint(
          '⚠️ [_salvarMedicoSemSair] NENHUMA disponibilidade única capturada! Total na lista: ${disponibilidades.length}');
      final unicasNaListaOriginal = disponibilidades
          .where((d) => d.tipo == 'Única' && d.medicoId == _medicoId)
          .length;
      debugPrint(
          '   🔍 Disponibilidades únicas encontradas na lista original: $unicasNaListaOriginal');
      if (unicasNaListaOriginal > 0) {
        debugPrint(
            '   ⚠️ PROBLEMA CRÍTICO: Há $unicasNaListaOriginal disponibilidades únicas na lista mas nenhuma foi capturada!');
        // Tentar capturar diretamente da lista original como fallback
        for (final disp in disponibilidades
            .where((d) => d.tipo == 'Única' && d.medicoId == _medicoId)) {
          debugPrint(
              '     - ID=${disp.id}, data=${disp.data.day}/${disp.data.month}/${disp.data.year}');
    ...
    debugPrint(
        '🔍 [_salvarMedicoSemSair] Removendo ${disponibilidades.length - disponibilidadesSemUnicas.length} disponibilidades únicas da lista antes de salvarMedicoCompleto');
    debugPrint(
        '🔍 [_salvarMedicoSemSair] Restam ${disponibilidadesSemUnicas.length} disponibilidades (séries/outras) para processar');
    ...
    final medico = Medico(
      ...
      observacoes: observacoesController.text, // Captura observações
      disponibilidades:
          disponibilidadesSemUnicas, // Adiciona as disponibilidades SEM as únicas (compatibilidade)
    );
    ...
    try {
      ...
      // CORREÇÃO: Usar a lista capturada no início, não a lista atual (pode ter sido alterada)
      debugPrint(
          '💾 [_salvarMedicoSemSair] Total de disponibilidades na lista atual: ${disponibilidades.length}');
      debugPrint(
          '💾 [_salvarMedicoSemSair] Usando ${disponibilidadesUnicasParaSalvar.length} disponibilidades únicas capturadas no início');

      final disponibilidadesUnicas = disponibilidadesUnicasParaSalvar;

      debugPrint(
          '💾 [_salvarMedicoSemSair] Salvando ${disponibilidadesUnicas.length} disponibilidades únicas para médico $_medicoId');
      ...
      for (final disp in disponibilidadesUnicas) {
        try {
          // CORREÇÃO CRÍTICA: Garantir que a disponibilidade única tem um ID válido permanente
          // Verificar se ID é temporário e substituir por permanente (IGUAL AO _salvarMedico)
          String idParaSalvar = disp.id;
          if (idParaSalvar.isEmpty ||
              idParaSalvar.startsWith('temp_') ||
              idParaSalvar.endsWith('-0')) {
            ...
            idParaSalvar = '${timestamp}_${_medicoId}_$dataStr';
            debugPrint(
                '⚠️ [_salvarMedicoSemSair] Disponibilidade única com ID vazio/temporário (${disp.id}), gerando novo permanente: $idParaSalvar');
          }
          ...
          final dataMap = dispComId.toMap();
          debugPrint(
              '💾 [_salvarMedicoSemSair] Salvando disponibilidade única: ID=$idParaSalvar, data=${disp.data.day}/${disp.data.month}/${disp.data.year}, ano=$ano, horários=${disp.horarios}');

          await disponibilidadesRef.doc(idParaSalvar).set(dataMap);
          unicasSalvas++;
          debugPrint(
              '✅ [_salvarMedicoSemSair] Disponibilidade única salva com sucesso: $idParaSalvar');
        } catch (e, stackTrace) {
          // CORREÇÃO: Reportar erro ao salvar disponibilidade única
          unicasErros++;
          debugPrint(
              '❌ [_salvarMedicoSemSair] Erro ao salvar disponibilidade única ${disp.id}: $e');
          debugPrint('   Stack trace: $stackTrace');
      ...
      debugPrint(
          '💾 [_salvarMedicoSemSair] Resultado: $unicasSalvas salvas, $unicasErros erros de ${disponibilidadesUnicas.length} disponibilidades únicas');

      // CORREÇÃO: Se deveria ter salvo disponibilidades únicas mas não salvou nenhuma,
      // mostrar aviso (pode indicar problema na lista)
      if (unicasSalvas == 0 && disponibilidadesUnicas.isNotEmpty) {
        debugPrint(
            '⚠️ [_salvarMedicoSemSair] AVISO CRÍTICO: Deveria ter salvo ${disponibilidadesUnicas.length} disponibilidades únicas mas nenhuma foi salva!');
      } else if (unicasSalvas > 0) {
        debugPrint(
            '✅ [_salvarMedicoSemSair] SUCESSO: $unicasSalvas disponibilidades únicas foram salvas!');
      }
